...
class Engine {
    ...
    run() {
        ...
        while (scenario.shouldIterate()) {
			...
			iterationLog += `  Current Population Size: ${this.currentPopulation.individuals.length}\n  New Population Size: ${this.newPopulation.individuals.length}\n`;
			this.rollIteration();
			...
			for (let topIdx = 0; topIdx < top5.individuals.length; topIdx++) {
				let topInd = top5.individuals[topIdx];
				iterationLog += `    [${topIdx+1}] ${topInd}  value: ${topInd.evaluate()}\n`;
			}
